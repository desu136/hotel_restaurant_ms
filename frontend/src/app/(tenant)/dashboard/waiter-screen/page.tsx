        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5 min-h-0 pr-1">
          {orders.map(order => {
            ...
            return (
              <div
                key={order.id}
                className={`flex h-70 flex-col shadow-lg rounded-2xl overflow-hidden transition-all ${isLate
                  ? "border-red-500/30 ring-1 ring-red-500/10"
                  : "border-[var(--surface-border)]"
                  }`}
              >
                {/* Card Header */}
                <div className={`px-4 py-2 border-b flex justify-between items-start gap-2 ${isLate ? "border-red-500/20" : "border-[var(--surface-border)]"
                  }`}>
                  <div>
                    <span className="text-[12px] bg-[var(--foreground)] text-[var(--background)] p-1 rounded-lg font-bold ">ORDER {order.order_number ? `#${order.order_number} (${order.id.slice(-6).toUpperCase()})` : `#${order.id.slice(-6).toUpperCase()}`}</span>
                  ...
                  <div className="flex flex-col items-end">
                    <span className={`flex items-center gap-1 text-xs font-semibold ${isLate ? "text-red-600 font-bold" : "text-amber-400"
                      }`}>
                      <Clock className="w-3.5 h-3.5" />
                      {elapsed}m ago
                    </span>
                    {isLate && (
                      <span className="text-[9px] text-red-700 font-black tracking-wider flex items-center gap-0.5 mt-0.5 uppercase">
                ...
                {/* Items */}
                <div className="flex-1 p-2 space-y-2 max-h-38 overflow-y-auto">
                  {order.items.map((it, idx) => (
                    <div key={idx} className="flex items-start gap-2 text-xs">
                      <span className="font-black shrink-0">{it.quantity}×</span>
                      <span className="font-medium">{it.menu_item.display_name}</span>
                    </div>
                  ))}
                </div>

                {/* Footer status indicator */}
                <div className="px-4 py-2.5 bg-[var(--foreground)] border-t border-[var(--surface-border)] flex justify-between items-center text-[13px] text-[var(--background)]">
                  <span>DINE_IN</span>
                  <span className="px-2 py-0.5  text-emerald-400 border border-emerald-500/20 rounded-md font-bold uppercase tracking-wider">
                    READY TO SERVE
                  </span>
                </div>
              </div>
