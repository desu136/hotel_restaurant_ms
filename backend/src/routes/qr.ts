...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
      ...
      const existingQr = await prisma.qRCode.findFirst({ where: { table_id } });
      if (existingQr) {
      ...
      const qrCode = await prisma.qRCode.create({
        data: { table_id, token, status: 'ACTIVE' },
      });

      return res.status(201).json({
        success: true,
        data: { ...qrCode, qrCodeUrl: qrCodeDataUrl, codeString: qrString },
      });
    } catch (error) {
      next(error);
    }
...
// Owners see all QR codes for the restaurant; branch staff only see their branch's QR codes.
router.get(
  '/list/:restaurantId',
  requireRole(...MANAGER_ROLES),
  async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
      ...
      const whereFilter = isOwner
        ...
        : { table: { branch_id: req.user!.branchId || '' } };
      ...
      const qrCodesWithUrls = await Promise.all(
        qrCodes.map(async (qr) => {
          ...
          return { ...qr, qrCodeUrl, codeString: qrString };
        })
      );

      return res.json({ success: true, data: qrCodesWithUrls });
    } catch (error) {
      next(error);
    }
  }
  ...
  async (req: Request, res: Response, next: NextFunction): Promise<any> => {
    try {
      ...
      const qrCode = await prisma.qRCode.findUnique({
        where: { id },
        include: { table: { select: { branch_id: true } } },
      });
